...
contract DEFYGenesisInvite is ERC721, ERC721Enumerable, Pausable, AccessControl, Ownable, IDEFYGenesisInvite {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return _tokenMetadata[tokenId].inviteState == InviteState.SPENT ?
            string(abi.encodePacked(_inviteBaseURI, Strings.toString(tokenId), '_spent.json')) :
            string(abi.encodePacked(_inviteBaseURI, Strings.toString(tokenId), '.json'));
    }
